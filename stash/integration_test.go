...
const (
	...
	// stashDomain = "https://gostash.acme.org/"
	// stashDomain = "https://gostash.dev.wkp.weave.works"
	stashDomain = "gostash.com"
...
var _ = Describe("Stash Provider", func() {
	...
	BeforeSuite(func() {
		...
		if orgName := os.Getenv("STASH_ORGANIZATION"); len(orgName) != 0 {
			testOrgName = orgName
		}

		if gitProviderUser := os.Getenv("STASH_USER"); len(gitProviderUser) != 0 {
			testUserName = gitProviderUser
		}

		if teamName := os.Getenv("STASH_TEAM_NAME"); len(teamName) != 0 {
	...
	It("should list the available organizations the user has access to", func() {
		...
		internal := getOrg.APIObject().(*Group)
	...
	It("should be possible to create a group project", func() {
		...
		getSpec := newStashProjectSpec(getRepo.APIObject().(*scm.Repository))
		postSpec := newStashProjectSpec(repo.APIObject().(*scm.Repository))
