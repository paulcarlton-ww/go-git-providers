...
func newClient(c *scm.Client, domain string, sshDomain string, destructiveActions bool) *Client {
	sClient := &stashClientImpl{c, destructiveActions}
	ctx := &clientContext{sClient, domain, sshDomain, destructiveActions}
	return &Client{
		clientContext: ctx,
...
// SupportedDomain returns the domain endpoint for this client, e.g. "gostash.com" or
...
// Raw returns the Go Stash client (github.com/xanzy *Client)
